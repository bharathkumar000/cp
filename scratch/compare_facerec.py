...
for dirpath, dirnames, filenames in os.walk(facerec_fusion):
    if any(p in dirpath for p in ['.git', '.next', 'node_modules', 'venv']):
        continue
    for filename in filenames:
        fusion_file = os.path.join(dirpath, filename)
        rel_path = os.path.relpath(fusion_file, facerec_fusion)
        root_file = os.path.join(facerec_root, rel_path)
...
print(f"Files only in Fusion-X/Facerecognition: {len(only_in_fusion)}")
